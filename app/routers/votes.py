...
async def create_vote(
    ...
    """check if vote exist"""
    vote = await get_filter_row(
        db, Vote, post_id=data.post_id, user_id=validated_user_id
    )

    """dir=1 - adding vote, dir=0 - deleting vote"""
    if not vote:
        if data.dir == 1:
            await add_row(db, Vote(post_id=data.post_id, user_id=validated_user_id))
        ...
        return VoteExc.http409(data.post_id, validated_user_id)
